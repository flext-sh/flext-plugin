...
FLEXT Plugin - Plugin System for FLEXT Platform
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-plugin` self).
...
- Public extensions exposed by this project: `FlextPluginPlatform`, `FlextPluginSettings`, `FlextPluginDiscovery`, `FlextPluginModels`, `FlextPluginHotReload`, `FlextPluginApi` (+10 more).
