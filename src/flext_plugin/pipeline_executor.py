...
class PipelineStep:
    ...
    def __init__(self, step_id: str, plugin_id: str, step_type: PluginType, configuration: dict[str, Any] | None = None, dependencies: list[str] | None = None) -> None:
...
class Pipeline:
    ...
    def __init__(self, pipeline_id: str, name: str, steps: list[PipelineStep], configuration: dict[str, Any] | None = None) -> None:
...
class RealPluginPipelineExecutor:
    ...
    async def execute_pipeline(self, pipeline: Pipeline, user_id: str | None = None, environment: str = "development", global_variables: dict[str, Any] | None = None) -> dict[str, Any]:
        execution_id = str(uuid.uuid4())

        # Create execution context
        context = DataFlowContext(pipeline_id=pipeline.pipeline_id,
        ...
        try:
            ...
            execution_id,
                ...
                "final_data":
                    final_data.get_data_summary() if final_data else None,:
                "output_data":
            final_data.data if final_data else None,:
    ...
    async def _execute_step(self, step: PipelineStep, flow_manager: DataFlowManager, ) -> None:
        ...
        step_result = flow_manager.start_step_execution(step.step_id,
            step.plugin_id,
            step.step_type,
        )
        ...
        try:
            ...
            output = await self._execute_transformer(plugin,
                    step,
                    input_data,
                    execution_context,
                )
            elif step.step_type == PluginType.LOADER:
            output = await self._execute_loader(plugin,
                    step,
                    input_data,
                    execution_context,
                )
            else:
            msg = f"Unsupported plugin type://{self.target_ldap_host}:{self.target_ldap_port}"
                {step.step_type}"
                raise PluginError(msg,
                    plugin_id=step.plugin_id,
    ...
    async def _execute_extractor(self, plugin: PluginInterface, step: PipelineStep, context: dict[str, Any], ) -> Any:
    ...
    async def _execute_transformer(self, plugin: PluginInterface, step: PipelineStep, input_data: Any, context: dict[str, Any], ) -> Any:
    ...
    async def _execute_loader(self, plugin: PluginInterface, step: PipelineStep, input_data: Any, context: dict[str, Any], ) -> Any:
    ...
    def _prepare_execution_context(: self, step: PipelineStep, flow_manager: DataFlowManager,  ) -> dict[str, Any]:
        ...
        context.model_copy(update={"step_id":
            step.step_id, "step_config": step.configuration},
        )
