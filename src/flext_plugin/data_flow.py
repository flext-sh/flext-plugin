...
class DataFlowContext(DomainBaseModel):
    ...
    # Data flow tracking
    source_step_id: str | None = Field(default=None,)
        default=None,
...
class DataPacket(DomainBaseModel):
    ...
    model_config = {"frozen":
            False}  # Allow mutations for data transformation
    ...
    def update_data(: self, new_data: Any, plugin_id: str, processing_info: dict[str, Any] | None = None, ) -> None:
        self.data = new_data
...
class PipelineDataFlow(DomainBaseModel):
    ...
    total_steps:
            int = Field(default=0, description="Total number of steps in pipeline")
...
class DataFlowManager:
    ...
    def complete_step_execution(: self, step_result: StepResult, output_data: Any = None, error: str | None = None,  ) -> None:
