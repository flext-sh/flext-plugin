...
class DataFlowContext(BaseModel):
    ...
    source_step_id: str | None = Field(
        default=None, description="Source step that produced this data"
    )
    data_lineage: list[str] = Field(
        default_factory=list, description="Chain of steps that processed this data"
    )

    # Metadata
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    user_id: str | None = Field(
        default=None, description="User who initiated the pipeline"
    )
    ...
    step_config: dict[str, Any] = Field(
        default_factory=dict, description="Current step configuration"
    )
    global_variables: dict[str, Any] = Field(
        default_factory=dict, description="Global pipeline variables"
    )
...
class DataPacket(BaseModel):
    ...
    schema: dict[str, Any] | None = Field(
        default=None, description="Data schema information"
    )

    # Metadata
    packet_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique packet identifier"
    )
    size_bytes: int | None = Field(default=None, description="Data size in bytes")
    row_count: int | None = Field(
        default=None, description="Number of records (if applicable)"
    )

    # Processing information
    source_plugin_id: str | None = Field(
        default=None, description="Plugin that generated this data"
    )
    source_plugin_type: PluginType | None = Field(
        default=None, description="Type of source plugin"
    )
    ...
    validation_errors: list[str] = Field(
        default_factory=list, description="Data validation errors"
    )
    quality_metrics: dict[str, Any] = Field(
        default_factory=dict, description="Data quality metrics"
    )
...
class StepResult(BaseModel):
    ...
    output_data: DataPacket | None = Field(
        default=None, description="Output data packet"
    )
    ...
    def mark_completed(
        self, output_data: DataPacket | None = None, error: str | None = None
    ) -> None:
...
class PipelineDataFlow(BaseModel):
    ...
    step_results: list[StepResult] = Field(
        default_factory=list, description="Results of completed steps"
    )
    current_data: DataPacket | None = Field(
        default=None, description="Current data flowing through pipeline"
    )
    ...
    should_continue: bool = Field(
        default=True, description="Whether pipeline should continue execution"
    )
    abort_reason: str | None = Field(
        default=None, description="Reason for aborting pipeline"
    )
    ...
    def start_step(
        ...
        step_result = StepResult(
            step_id=step_id, plugin_id=plugin_id, plugin_type=plugin_type, success=False
        )
    ...
    def complete_step(
        self, step_result: StepResult, output_data: DataPacket | None = None
    ) -> None:
...
class DataValidator:
    ...
    def validate_data_packet(
        packet: DataPacket, expected_schema: dict[str, Any] | None = None
    ) -> list[str]:
...
class DataFlowManager:
    ...
    def __init__(
        ...
        self.pipeline_flow = PipelineDataFlow(
            pipeline_id=pipeline_id, execution_id=execution_id, context=context
        )
        self.validator = DataValidator()

    def create_data_packet(
        self, data: Any, plugin_id: str, plugin_type: PluginType
    ) -> DataPacket:
        """Create a new data packet."""
        packet = DataPacket(
            data=data, source_plugin_id=plugin_id, source_plugin_type=plugin_type
        )
    ...
    def complete_step_execution(
        self, step_result: StepResult, output_data: Any = None, error: str | None = None
    ) -> None:
        """Complete execution of a pipeline step."""
        if output_data is not None and error is None:
            ...
            data_packet = self.create_data_packet(
                output_data, step_result.plugin_id, step_result.plugin_type
            )
