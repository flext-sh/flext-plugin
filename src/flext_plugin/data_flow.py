...
between pipeline steps, including data validation, transformation tracking,
and step communication protocols.
"""
...
class DataFlowContext(BaseModel):
    ...
    # Data flow tracking
    source_step_id: str | None = Field(default=None, description="Source step that produced this data")
    data_lineage: list[str] = Field(default_factory=list, description="Chain of steps that processed this data")

    # Metadata
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    user_id: str | None = Field(default=None, description="User who initiated the pipeline")
    environment: str = Field(default="development", description="Execution environment")

    # Configuration and state
    pipeline_config: dict[str, Any] = Field(default_factory=dict, description="Pipeline configuration")
    step_config: dict[str, Any] = Field(default_factory=dict, description="Current step configuration")
    global_variables: dict[str, Any] = Field(default_factory=dict, description="Global pipeline variables")
...
class DataPacket(BaseModel):
    ...
    schema: dict[str, Any] | None = Field(default=None, description="Data schema information")

    # Metadata
    packet_id: str = Field(default_factory=lambda: str(uuid4()), description="Unique packet identifier")
    size_bytes: int | None = Field(default=None, description="Data size in bytes")
    row_count: int | None = Field(default=None, description="Number of records (if applicable)")

    # Processing information
    source_plugin_id: str | None = Field(default=None, description="Plugin that generated this data")
    source_plugin_type: PluginType | None = Field(default=None, description="Type of source plugin")
    ...
    quality_metrics: dict[str, Any] = Field(default_factory=dict, description="Data quality metrics")

    def update_data(self, new_data: Any, plugin_id: str, processing_info: dict[str, Any] = None) -> None:
    ...
    def add_validation_error(self, error: str) -> None:
        """Add a validation error to the packet."""
        if error not in self.validation_errors:
            self.validation_errors.append(error)

    def is_valid(self) -> bool:
        """Check if the data packet is valid (no validation errors)."""
        return len(self.validation_errors) == 0

    def get_data_summary(self) -> dict[str, Any]:
        """Get a summary of the data packet for logging and monitoring."""
        return {
    ...
    def _update_metrics(self) -> None:
        """Update size and row count metrics based on current data."""
        try:
            ...
            elif isinstance(self.data, dict) and "data" in self.data and isinstance(self.data["data"], list):
...
class StepResult(BaseModel):
    ...
    execution_result: PluginExecutionResult | None = Field(default=None, description="Plugin execution result")
    output_data: DataPacket | None = Field(default=None, description="Output data packet")
    ...
    def mark_completed(self, output_data: DataPacket | None = None, error: str | None = None) -> None:
...
class PipelineDataFlow(BaseModel):
    ...
    step_results: list[StepResult] = Field(default_factory=list, description="Results of completed steps")
    current_data: DataPacket | None = Field(default=None, description="Current data flowing through pipeline")

    # Context and configuration
    context: DataFlowContext = Field(description="Pipeline execution context")

    # Flow control
    should_continue: bool = Field(default=True, description="Whether pipeline should continue execution")
    abort_reason: str | None = Field(default=None, description="Reason for aborting pipeline")
    ...
    def start_step(self, step_id: str, plugin_id: str, plugin_type: PluginType) -> StepResult:
        """Start execution of a pipeline step."""
    ...
    def complete_step(self, step_result: StepResult, output_data: DataPacket | None = None) -> None:
        """Complete execution of a pipeline step."""
        ...
        if step_result.success:
            ...
            self.abort_reason = f"Step {step_result.step_id} failed: {step_result.error_message}"
...
class DataValidator:
    ...
    @staticmethod
    def validate_data_packet(packet: DataPacket, expected_schema: dict[str, Any] | None = None) -> list[str]:
        """Validate a data packet against an optional schema."""
        errors = []
        ...
        if expected_schema:
            schema_errors = DataValidator._validate_against_schema(packet.data, expected_schema)
...
class DataFlowManager:
    ...
    def create_data_packet(self, data: Any, plugin_id: str, plugin_type: PluginType) -> DataPacket:
    ...
    def get_current_data(self) -> DataPacket | None:
        """Get the current data flowing through the pipeline."""
