...
class TestFlextPluginSettings:
    def test_create_config(self) -> None:
        ...
        tm.that(config.model_dump() != {}, eq=True)
    ...
    def test_create_config_with_custom_values(self) -> None:
        ...
        tm.that(abs(config.timeout_seconds - 60.0) < 1e-9, eq=True)
    ...
    def test_update_timestamp(self) -> None:
        ...
        tm.that(abs(config.timeout_seconds - 45.0) < 1e-9, eq=True)
        tm.that(initial_timeout != config.timeout_seconds, eq=True)
