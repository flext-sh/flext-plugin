...
class TestFlextPluginRegistrationHandler:
    ...
    def test_handle_register_plugin_no_service_fails(self, mock_plugin: FlextPlugin) -> None:
    ...
    def test_handle_unregister_plugin_service_failure(
        ...
        plugin_name = "test-plugin"
        mock_plugin_service.unload_plugin.return_value = FlextResult.fail("Unload error")
...
class TestFlextPluginEventHandler:
    ...
    def test_handle_plugin_loaded_runtime_exception(
        ...
        try:
            ...
            assert not result.is_success
            assert "Failed to handle plugin loaded event: Attribute access error" in result.error
        finally:
...
class TestHandlerIntegration:
    ...
    def test_registration_and_event_handler_integration(self) -> None:
        ...
        registration_handler = FlextPluginRegistrationHandler(plugin_service=mock_service)
        ...
        # Unregister plugin
        unregister_result = registration_handler.handle_unregister_plugin("integration-test-plugin")
        assert unregister_result.is_success

        # Handle plugin unloaded event
        unloaded_result = event_handler.handle_plugin_unloaded("integration-test-plugin")
    ...
    def test_handler_error_consistency(self) -> None:
        ...
        assert not reg_result.is_success
