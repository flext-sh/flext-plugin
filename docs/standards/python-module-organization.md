...
- [📁 Module Structure & Responsibilities](#module-structure-responsibilities)
  - [Foundation Layer (src/flext_plugin/)](#foundation-layer-srcflext_plugin)
  - [Core Layer (src/flext_plugin/core/)](#core-layer-srcflext_plugincore)
...
**Responsibility**: Establish the plugin system's public interface and high-level
orchestration.
...
**Responsibility**: Define fundamental plugin types, status enums, and core discovery
logic.
...
**Responsibility**: Handle plugin system configuration, validation, and environment
management.
...
class DataProcessorPlugin(FlextPlugin):
    ...
    def __init__(
        self,
        settings: (FlextPluginModels.Config | None) = None,
        metadata: (FlextPluginModels.Metadata | None) = None,
        **kwargs,
    ) -> None:
        """
    ...
    def execute(self, data: dict) -> p.Result[dict]:
        ...
        Returns:
            r[dict]: Processing results containing:
...
def create_oracle_wms_plugin(settings: dict) -> p.Result[FlextPlugin]:
...
def create_oracle_tap_plugin(tap_config: dict) -> p.Result[FlextPlugin]:
...
**Last Updated**: August 3, 2025 **Target Audience**: FLEXT Plugin developers and
ecosystem contributors **Scope**: Python module organization for plugin system
development **Version**: 0.12.0-dev → 0.9.9 development guidelines for plugin
architecture
