...
- Plugin lifecycle is **explicit** — discover → load → register → execute → unregister;
  hot reload is managed through the facade.
- Registration/discovery is protocol-driven (`p.*`); depend on abstractions, not
