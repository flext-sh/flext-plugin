...
def _get_signature_object(func, as_instance, eat_self):
    ...
    if isinstance(func, type) and not as_instance:
        ...
        try:
            ...
            return None
    sig_func = partial(func, None) if eat_self else func
    try:
...
def _check_signature(func, mock, skipfirst, instance=False) -> None:
    sig = _get_signature_object(func, instance, skipfirst)
    if sig is None:
        return
    func, sig = sig

    def checksig(self, /, *args, **kwargs) -> None:
...
def _copy_func_details(func, funcopy) -> None:
...
def _set_async_signature(mock, original, instance=False, is_async_mock=False):
    ...
    name = original.__name__
    context = {"_checksig_": checksig, "mock": mock}
    src = f"""async def {name}(*args, **kwargs):
    _checksig_(*args, **kwargs)
...
def _setup_func(funcopy, mock, sig) -> None:
    funcopy.mock = mock
...
def _setup_async_mock(mock) -> None:
...
def _delegating_property(name):
    ...
    the_name = "_mock_" + name

    def _get(self, name=name, _the_name=the_name):
        sig = self._mock_delegate
        if sig is None:
            return getattr(self, _the_name)
        return getattr(sig, name)

    def _set(self, value, name=name, _the_name=the_name) -> None:
        sig = self._mock_delegate
        if sig is None:
...
def _check_and_set_parent(parent, value, name, new_name) -> bool:
    ...
    while parent_ is not None:
        ...
        if parent_ is value:
            return False
        parent_ = parent_._mock_new_parent
...
class NonCallableMock(Base):
    ...
    def __new__(
        ...
        return _safe_super(NonCallableMock, cls).__new__(new)
    ...
    def attach_mock(self, mock, attribute) -> None:
    ...
    def mock_add_spec(self, spec, spec_set=False) -> None:
        ...
        if spec is not None and not _is_list(spec):
            ...
            for attr in spec_list:
                ...
                if iscoroutinefunction(unwrapped_attr):
                    spec_asyncs.append(attr)

            spec = spec_list

        __dict__ = self.__dict__
        __dict__["_spec_class"] = spec_class
        __dict__["_spec_set"] = spec_set
        __dict__["_spec_signature"] = spec_signature
        __dict__["_mock_methods"] = spec
        __dict__["_spec_asyncs"] = spec_asyncs
    ...
    def __set_side_effect(self, value) -> None:
    ...
    def configure_mock(self, /, **kwargs) -> None:
    ...
    def __getattr__(self, name):
        ...
        if self._mock_methods is not None:
            if name not in self._mock_methods or name in _all_magics:
                msg = f"Mock object has no attribute {name!r}"
    ...
    def _extract_mock_name(self):
        name_list = [self._mock_new_name]
        parent = self._mock_new_parent
        last = self
        ...
        while parent is not None:
            ...
            name_list.append(parent._mock_new_name + dot)
            dot = "."
            if parent._mock_new_name == "()":
                dot = ""

            parent = parent._mock_new_parent

        name_list.reverse()
        first = last._mock_name or "mock"
        if len(name_list) > 1 and name_list[1] not in {"()", "()."}:
            first += "."
        name_list[0] = first
    ...
    def __setattr__(self, name, value) -> None:
        ...
        if (self._spec_set and self._mock_methods is not None and
            ...
            msg = f"Mock object has no attribute '{name}'"
            raise AttributeError(msg)
    ...
    def assert_not_called(self) -> None:
        ...
        if self.call_count != 0:
            msg = ("Expected '{}' to not have been called. Called {} times.{}".format(self._mock_name or "mock",
                      self.call_count,
                      self._calls_repr()))
    ...
    def assert_called_once(self) -> None:
        ...
        if self.call_count != 1:
            msg = ("Expected '{}' to have been called once. Called {} times.{}".format(self._mock_name or "mock",
                      self.call_count,
                      self._calls_repr()))
            raise AssertionError(msg)
    ...
    def assert_called_once_with(self, /, *args, **kwargs):
        ...
        """
        if self.call_count != 1:
            msg = ("Expected '{}' to be called once. Called {} times.{}".format(self._mock_name or "mock",
                      self.call_count,
                      self._calls_repr()))
            raise AssertionError(msg)
    ...
    def assert_has_calls(self, calls, any_order=False) -> None:
        ...
        if not_found:
            msg = (
                ...
                "found {!r} instead".format(self._mock_name or "mock",
    ...
    def _get_child_mock(self, /, **kw):
        ...
        new_name = kw.get("_new_name")
        if new_name in self.__dict__["_spec_asyncs"]:
            return AsyncMock(**kw)

        type_ = type(self)
        if issubclass(type_, MagicMock) and new_name in _async_method_magics:
            # Any asynchronous magic becomes an AsyncMock
            klass = AsyncMock
        elif issubclass(type_, AsyncMockMixin):
            if (new_name in _all_sync_magics or
                    (self._mock_methods and new_name in self._mock_methods)):
                # Any synchronous method on AsyncMock becomes a MagicMock
                klass = MagicMock
            else:
                klass = AsyncMock
        elif not issubclass(type_, CallableMixin):
            if issubclass(type_, NonCallableMagicMock):
                klass = MagicMock
            elif issubclass(type_, NonCallableMock):
                klass = Mock
        else:
            klass = type_.__mro__[1]
        return klass(**kw)

    def _calls_repr(self) -> str:
        r"""Renders self.mock_calls as a string.
...
class CallableMixin(Base):
    ...
    def __init__(self, spec=None, side_effect=None, return_value=DEFAULT,
                 ...
                 _spec_state=None, _new_name="", _new_parent=None, **kwargs) -> None:
    ...
    def _mock_check_sig(self, /, *args, **kwargs) -> None:
    ...
    def _increment_mock_call(self, /, *args, **kwargs) -> None:
        self.called = True
        self.call_count += 1
        ...
        call_ = _Call((args, kwargs), two=True)
        self.call_args = call_
        self.call_args_list.append(call_)
        ...
        new_parent = self._mock_new_parent
        while new_parent is not None:

            # handle method_calls:
            if do_method_calls:
                new_parent.method_calls.append(_Call((method_call_name, args, kwargs)))
                do_method_calls = new_parent._mock_parent is not None
                if do_method_calls:
                    method_call_name = new_parent._mock_name + "." + method_call_name

            # handle mock_calls:
            this_mock_call = _Call((mock_call_name, args, kwargs))
            new_parent.mock_calls.append(this_mock_call)

            if new_parent._mock_new_name:
                dot = "" if is_a_call else "."
                is_a_call = new_parent._mock_new_name == "()"
                mock_call_name = new_parent._mock_new_name + dot + mock_call_name

            # follow the parental chain:
            new_parent = new_parent._mock_new_parent
...
def _check_spec_arg_typos(kwargs_to_check) -> None:
    typos = ("autospect", "auto_spec", "set_spec")
    for typo in typos:
        if typo in kwargs_to_check:
            msg = f"{typo!r} might be a typo; use unsafe=True if this is intended"
            raise RuntimeError(
                msg,
            )
...
class _patch:
    ...
    def __init__(
        ...
        if new_callable is not None:
            if new is not DEFAULT:
                msg = "Cannot use 'new' and 'new_callable' together"
                raise ValueError(
                    msg,
                )
            if autospec is not None:
                msg = "Cannot use 'autospec' and 'new_callable' together"
                raise ValueError(
                    msg,
                )
        if not unsafe:
    ...
    def get_original(self):
        ...
        if not self.create and original is DEFAULT:
            msg = f"{target} does not have the attribute {name!r}"
            raise AttributeError(
                msg,
            )
    ...
    def __enter__(self):
        ...
        if spec is not None and autospec is not None:
            msg = "Can't specify spec and autospec"
            raise TypeError(msg)
        if ((spec is not None or autospec is not None) and
            spec_set not in {True, None}):
            msg = "Can't provide explicit spec_set *and* spec or autospec"
            raise TypeError(msg)
        ...
        if new is DEFAULT and autospec is None:
            ...
            if spec is not None or spec_set is not None:
                if original is DEFAULT:
                    msg = "Can't use 'spec' with create=True"
                    raise TypeError(msg)
            ...
            if spec is not None:
                kwargs_["spec"] = spec
            if spec_set is not None:
                kwargs_["spec_set"] = spec_set

            # add a name to mocks
            if (isinstance(Klass, type) and
                issubclass(Klass, NonCallableMock) and self.attribute):
                kwargs_["name"] = self.attribute
            ...
            if original is DEFAULT:
                msg = "Can't use 'autospec' with create=True"
                raise TypeError(msg)
            ...
            msg = "Can't pass kwargs to a mock we aren't creating"
            raise TypeError(msg)
...
def _get_target(target):
    try:
        ...
        msg = f"Need a valid target to patch. You supplied: {target!r}"
        raise TypeError(
            msg)
...
def _patch_object(
    ...
    if type(target) is str:
        msg = f"{target!r} must be the actual object to be patched, not a str"
        raise TypeError(
            msg,
        )
...
def _patch_multiple(target, spec=None, create=False, spec_set=None,
    ...
    if not kwargs:
        msg = "Must supply at least one keyword argument with patch.multiple"
        raise ValueError(
            msg,
        )
...
class _patch_dict:
    ...
    def __init__(self, in_dict, values=(), clear=False, **kwargs) -> None:
...
_magics = {
    f"__{method}__" for method in
    f"{magic_methods} {numerics} {inplace} {right}".split()
}
...
def _set_return_value(mock, method, name) -> None:
...
class MagicMixin(Base):
    ...
    def _mock_set_magics(self) -> None:
        ...
        these_magics -= set(type(self).__dict__)

        type_ = type(self)
        for entry in these_magics:
            setattr(type_, entry, MagicProxy(entry, self))
...
class MagicMock(MagicMixin, Mock):
    ...
    def reset_mock(self, /, *args, return_value: bool = False, **kwargs) -> None:
        if (
            return_value
            and self._mock_name
...
class AsyncMockMixin(Base):
    ...
    async def _execute_mock_call(self, /, *args, **kwargs):
        ...
        call_ = _Call((args, kwargs), two=True)
        self.await_count += 1
        self.await_args = call_
        self.await_args_list.append(call_)
    ...
    def assert_awaited_with(self, /, *args, **kwargs) -> None:
        ...
        if self.await_args is None:
            expected = self._format_mock_call_signature(args, kwargs)
            msg = f"Expected await: {expected}\nNot awaited"
            raise AssertionError(msg)

        def _error_message():
            return self._format_mock_failure_message(args, kwargs, action="await")
    ...
    def assert_any_await(self, /, *args, **kwargs) -> None:
        ...
        if cause or expected not in _AnyComparer(actual):
            expected_string = self._format_mock_call_signature(args, kwargs)
            msg = f"{expected_string} await not found"
            raise AssertionError(
                msg,
            ) from cause

    def assert_has_awaits(self, calls, any_order=False) -> None:
        ...
        for kall in expected:
            try:
                ...
                not_found.append(kall)
        if not_found:
            msg = f"{tuple(not_found)!r} not all found in await list"
            raise AssertionError(
                msg,
            ) from cause
...
def create_autospec(spec, spec_set=False, instance=False, _parent=None,
    ...
    if _is_instance_mock(spec):
        msg = (
            f"Cannot autospec a Mock object. "
                               f"[object={spec!r}]"
        )
        raise InvalidSpecError(msg)
    ...
    kwargs_ = {"spec": spec}
    if spec_set:
        kwargs_ = {"spec_set": spec}
    elif spec is None:
        # None we mock with a normal mock without a spec
        kwargs_ = {}
    if kwargs_ and instance:
        kwargs_["_spec_as_instance"] = True
    if not unsafe:
        _check_spec_arg_typos(kwargs)
    ...
    mock = Klass(parent=_parent, _new_parent=_parent, _new_name=new_name,
                 name=_name, **kwargs_)
    ...
    for entry in dir(spec):
        ...
        if not isinstance(original, FunctionTypes):
            ...
            child_klass = AsyncMock if iscoroutinefunction(original) else MagicMock
...
def _must_skip(spec, entry, is_type):
    ...
    for klass in spec.__mro__:
        ...
        if isinstance(result, (staticmethod, classmethod)):
            return False
        if isinstance(result, FunctionTypes):
...
class _SpecState:
    ...
    def __init__(self, spec, spec_set=False, parent=None,
                 name=None, ids=None, instance=False) -> None:
        self.spec = spec
        self.ids = ids
        self.spec_set = spec_set
        self.parent = parent
...
def mock_open(mock=None, read_data=""):
    ...
    """
    read_data_ = _to_stream(read_data)
    state = [read_data_, None]

    def _readlines_side_effect(*args, **kwargs):
        if handle.readlines.return_value is not None:
            return handle.readlines.return_value
        return state[0].readlines(*args, **kwargs)

    def _read_side_effect(*args, **kwargs):
        if handle.read.return_value is not None:
            return handle.read.return_value
        return state[0].read(*args, **kwargs)

    def _readline_side_effect(*args, **kwargs):
        yield from _iter_side_effect()
        while True:
            yield state[0].readline(*args, **kwargs)
    ...
    def _exit_side_effect(exctype, excinst, exctb) -> None:
        handle.close()
    ...
    state[1] = _readline_side_effect()
    handle.readline.side_effect = state[1]
    ...
    def reset_data(*args, **kwargs):
        state[0] = _to_stream(read_data)
        if handle.readline.side_effect == state[1]:
...
class ThreadingMixin(Base):
    ...
    def __init__(self, *args, timeout=_timeout_unset, **kwargs) -> None:
        super().__init__(*args, **kwargs)
    ...
    def wait_until_called(self, *, timeout=_timeout_unset) -> None:
    ...
    def wait_until_any_call_with(self, *args, **kwargs) -> None:
